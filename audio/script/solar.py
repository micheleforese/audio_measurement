...
def solar(
    ...
    # "TCPIP0::192.168.10.233::inst0::INSTR"
    resource: TCPIPInstrument = rm.open_resource(devices[-1])
    resource.read_termination = "\n"

    # resource.write("RESistance:STATe?")
    # response = resource.read(encoding="utf-8")
    # console.print(response)

    table = Table(title="Solar Panel Characterization")
    table.add_column("Voltage Set", justify="right")
    table.add_column("Voltage Read", justify="right")
...
@click.command()
def lux() -> None:
    lux_meter = ExtechLuxMeter(device_port="/dev/ttyUSB1")
    data: int | None = lux_meter.read()
    console.log(data)


class ExtechLuxMeter:
...
def lux_find(lux: int) -> None:
    ...
    table.add_column("Lux [Lux]", justify="right")
    table.add_column("Error [delta]", justify="right")
    table.add_column("Proportional Term [V]", justify="right")
    table.add_column("Integral Term [V]", justify="right")
    table.add_column("Differential Term [V]", justify="right")
    table.add_column("Error [%]", justify="right")

    live = Live(table, console=console, vertical_overflow="visible")
    ...
    while not lux_found:
        ...
        if lux_read is None:
            console.log("ERROR: lux read none.")
            sys.exit()
...
class KoradPowerSupplyKA6003P:
    ...
    def __init__(
        ...
        try:
            ...
            console.log("ERROR: Korad port missing.")
